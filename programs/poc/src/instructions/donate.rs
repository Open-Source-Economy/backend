...
pub fn handler(ctx: Context<Donate>, quote_amount: u64) -> Result<()> {
    ...
    let mint_data: MintData = abc.get_mint_data(0, quote_amount, project_token_mint.supply)?;
    ...
    // Mint token to the project treasury
    token::mint_to(
        CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            token::MintTo {
                mint: project_token_mint.to_account_info(),
                to: ctx.accounts.project_token_treasury.to_account_info(),
