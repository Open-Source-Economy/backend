...
async function sendToMantainer(gitIssueURL: string) {
  const [
    escrowAgent,
    funder,
    mantainer,
  ...
  const tx = await escrowContract
    .connect(escrowAgent)
    .sendToMantainer(gitIssueURL, mantainer);
  ...
  const escrowContractBalance =
    await ethers.provider.getBalance(contractAddress);
  console.log(ethers.formatEther(escrowContractBalance), "ETH");
  const mantainerBalance = await ethers.provider.getBalance(mantainer.address);
  console.log(ethers.formatEther(mantainerBalance), "ETH");
}
